...
- 문자 근거는 URL을 제거해 전달받은 현재 본문에 실제로 있는 표현이어야 한다. 다른 문자나 페이지의 문구를 복사하지 않는다. `결제 성공`, `환불되었습니다` 같은 완료 안내나 `앱을 설치하지 마세요` 같은 부정 표현을 행동 요구로 해석하지 않는다.
...
- 두 `answer` 중 하나라도 `null`이면 분석 미완료를 우선 표시하고, 이미 확보한 근거와 실패 이유를 함께 보존한다.
- 필요한 분석이 모두 완료된 경우에만 `result=false`를 의심으로 표시하며 개별 `answer`와 최종 결과를 구분한다.
...
- 이 표시 정책의 실제 FE 적용은 후속 연동 작업이며 현재 완료된 것으로 간주하지 않는다.
