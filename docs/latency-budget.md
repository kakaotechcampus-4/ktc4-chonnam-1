...
현재 AI 메시지 시나리오 테스트에서는 메시지 추출과 위험 신호 분석의 제한을
각각 30초로 완화한다. 두 단계가 공유하는 SDK 클라이언트도 30초로 설정한다.
순차 LLM 호출 예산만 최대 60초이며 아래 운영 목표 예산과 다르다.
격리 페이지 분석과 설명 생성은 기존 2초를 유지한다. BE의 콜백 제한은 별도이므로
이 설정은 BE 전체 요청이 60초를 기다린다는 의미가 아니다.
