...
- 구현: a97850c, 6afa423, f36a43b, cc5343a, ca037aa.
- AI 전체 테스트 687개 통과, HTML/LLM/최종 함수 집중 테스트 135개 통과.
- 로컬 합성 측정: 정상 입력 1.17ms, 깊이64 1.54ms, 폼200개 21.80ms, label400개 73.02ms. AWS 실측이나 엄격한 시간 상한 증명이 아니다.
- 중첩 폼 10,673byte 입력의 출력이 기존 1,037,395byte에서 259,572byte로 제한됐으며 PARTIAL_CONTENT로 반환된다.
- 구현 보정: 앞의 큰 후보를 담을 수 없으면 뒤의 작은 후보를 원문 순서로 보존할 수 있다. 완전한 접두 목록을 보장하지 않으며 부분 상태를 유지한다.
- 개별 커밋과 테스트를 거쳐 하나의 검사 흐름으로 독립 리뷰했다. 원격 계약·범용 DOM 해석 확대·인프라 구현은 수행하지 않았다.
