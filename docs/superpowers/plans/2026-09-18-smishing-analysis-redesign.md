...
Run: `grep -rn "격리 분석을 호출하거나\|콜백에는 의존하지 않는다\|프로토타입에서 제외\|4초 데드라인\|자체 sandbox[와나만도]\|프로토타입 평가에서 제외\|미확정이면\|후속 확정된" docs/ ai/ CLAUDE.md --include=*.md | grep -vE "docs/(superpowers|adr)/"`
