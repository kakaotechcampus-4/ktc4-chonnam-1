...
- **관측**은 백엔드가 주소와 응답에서 직접 확인한 정보다. 관측이 없으면 **판정하지 않고 판단을 보류한다.**
- 관측이 하나도 없을 때에 한해, AI가 파악한 **문자의 형식**을 판정과 분리된 참고 칸에 표시할 수 있다. 참고는 판정이 아니며 결과 상태를 바꾸지 않는다(3절 참고 정보 표시 규칙).
...
3. 백엔드는 내용과 URL 개수를 확인한다. 내용 자체가 너무 짧으면 다시 요청한다.
4. **주소가 없어도 잘못된 입력으로 되돌리지 않는다.** 접수한 뒤 관측할 대상이 없다는 사실을 알리고 판단 보류로 답한다. 주소 추가는 요구가 아니라 그 화면에서 제안하는 선택지다.
5. URL이 하나면 백엔드가 주소를 확인하고 공식 주소 목록과 비교한다.
6. 4초 안에 확인한 정보에 따라 결과, 부분 결과, 판단 보류 또는 처리 실패를 안내한다.
7. 사용자는 상세 근거를 볼 수 있다. 위험 징후가 있으면 공식 대응 경로도 제공한다.
...
| 입력 부족 | 문자 전체를 다시 요청 | 다시 입력 |
| 주소 없음 | 확인할 주소가 보이지 않는다는 사실. 판단 보류로 수렴 | 주소가 있는 부분 추가·상세 보기 |
...
| 판단 보류 | 확인하지 못한 이유. 관측이 0건일 때만 AI 유형 참고를 **판정과 분리된 칸**에 표시 | 공식 경로·상세 보기 |
...
판단 보류 화면의 참고 칸은 AI가 만든 문장이다. 이 문장이 사실상의 판정으로 읽히지 않도록 네 가지 제약을 둔다.

1. **어휘 차단** — 참고 문구에 `위험`, `안전`, `사기`, `정상`, `스미싱`, `의심` 같은 판정 어휘가 들어 있으면 문구를 쓰지 않고 칸 전체를 그리지 않는다. `docs/security-checklist.md`의 “LLM 출력이 그대로 카드에 나가기 전 가드” 항목에 해당한다.
2. **상태 불변** — 참고 문구의 유무와 내용은 결과 상태(`판단 보류`)와 버튼 구성을 바꾸지 못한다. 결정적 로직이 정한 상태를 AI가 흔들 수 없다.
3. **없으면 안 그림** — AI가 실패하거나 시간을 넘기면 참고 칸 자체를 렌더하지 않는다. 판단 보류 안내는 참고 문구 없이도 완결되어야 한다.
4. **형식 서술만** — 문자의 **형태**만 말하고 발신자의 의도나 결과를 말하지 않는다. (가능: “택배 배송 안내 형식을 띱니다” / 불가: “사칭으로 보입니다”)

참고 칸에는 이것이 판정이 아니라는 사실을 함께 표시한다.
...
| BE 응답 변환 | 판정 결과에 맞는 템플릿을 골라 값을 채우고 카카오 응답 JSON으로 변환. 참고 문구의 어휘 가드도 여기서 강제 |
| AI | 문자 해석, URL 추출 보조, 결과 설명. 관측 0건일 때 유형 참고 제공. 최종 판정에는 관여하지 않음 |
...
FE가 상세 화면을 만들려면 BE로부터 결과 상태, 요약, 확인한 내용, 확인하지 못한 내용, 관측 시각·출처, 다음 행동, 상세 진입 토큰이 필요하다. 참고 문구를 쓰는 경우에는 그것이 AI 문장인지 템플릿 문장인지도 구분되어야 한다. 실제 필드명은 후속 협의에서 정한다.

## 6. 검토가 필요한 결정

1. 4초 안에 결과가 없을 때 판단 보류로 끝낼지, 나중에 결과를 다시 전달할지?
2. URL이 없을 때 AI 유형 참고를 노출하는 것이 `CLAUDE.md` 절대 원칙 1(“판정은 결정적 로직이 한다”)과 양립하는가?
3. 부분 결과, 판단 보류, 처리 실패를 백엔드가 어떤 상태로 구분해 줄지?
4. “이 문자 확인해줘” 같은 채팅을 어디서 문자 확인 요청으로 구분할지?
5. FE가 사용할 결과 데이터 형식을 어디에서 함께 관리할지?
6. 전화번호만 있는 문자를 두 번째 관측 대상(공식 고객센터 번호 목록 대조)으로 승격할지? 승격하면 URL 없이도 결정적 판정이 가능해지지만 `docs/PRD.md`의 1차 범위를 넓히는 결정이다.
